...
package shalom.bible.hymn.cion.favorite.page;
...
//		addInterstitialView();
	...
	public void addInterstitialView() {
    	if(interstialAd == null) {
        	AdInfo adInfo = new AdInfo("88tczf12");
//        	adInfo.setTestMode(false);
    ...
    private void startPlaying(String file_path) {
    	thumb_image.setImageResource(R.drawable.ic_action_pause);
...
//        getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
    ...
    private void stopPlaying() {
    	thumb_image.setImageResource(R.drawable.ic_action_play);
		...
		header_listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long arg3) {
						...
						}else if(ls.getPath().endsWith("mp4")){
							Toast.makeText(ct, ct.getString(R.string.toast_media_volume), Toast.LENGTH_SHORT).show();
							onPlay(isPlaying, ls.getPath());
							isPlaying = !isPlaying;
						}else{
	...
	private class HeaderListAdapter extends ArrayAdapter<HeaderList> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (p != null) {
				...
				thumb_image = (ImageView) v.findViewById(R.id.thumb_img);
				thumb_image.setFocusable(false);
				if (tt != null) {
					if(p.getTitle() != null){
					tt.setText(p.getTitle());
					}else{
...
//						}
//					});
				}else if(image != null && p.getPath().endsWith("mp4")) {
					tt.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							onPlay(isPlaying, p.getPath());
							isPlaying = !isPlaying;
						}
					});
